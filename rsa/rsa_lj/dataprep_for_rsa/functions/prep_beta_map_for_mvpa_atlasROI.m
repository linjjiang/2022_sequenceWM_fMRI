...
function prep_beta_map_for_mvpa_atlasROI(rootDir,rootOutDir,roiDir,idx)
...
% load ROIs
roi_files = dir(fullfile(roiDir,'*.nii')); % all the roi files %'mgs_*_'
 
% get roi names
roi_names = struct2cell(roi_files); roi_names = roi_names(1,:);
roi_names = cellfun(@(C) erase(C,'.nii'),roi_names,'UniformOutput',false);
...
for sb = idx %1:length(subDir) % for each subject
    ...
    for ss = 1:length(sessDir) % for each session under that subject
        ...
        clearvars -except idx rootDir rootOutDir subDir sessDir outputDir sb ss sub_id sess_id task_id roiDir roi_files roi_names
