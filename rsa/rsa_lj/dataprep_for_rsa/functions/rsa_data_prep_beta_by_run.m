...
function rsa_data_prep_beta_by_run(rootDir,idx)
...
good_cond = {'3WL','3TL','3WR','3TR','5.5WL','5.5TL','5.5WR','5.5TR'};
...
for ii = 1:length(uniq_roi) % for each roi
    for ss = 1:length(uniq_sess) % for each session
        ...
        for cc = 1:length(uniq_cond) % for each condition
                ...
                % if current condition is 3dva/5.5dva
                if strcmp(curr_cond(1),'3') % if 3 dva
                    curr_ecc = 'E1';
                else
                    curr_ecc = 'E2';
                end
...
% update the field name with eccentricity
        fn = [curr_roi ' | ' curr_sub ' | ' curr_sess ' | ' curr_run ' | ' curr_ecc];
...
load(fullfile(out_dir,'f16','responsePattern.mat'),'good_cond') % only load the condition labels
...
% fourth model: spatial (eccentricity) pattern
...
save(fullfile(out_dir,'model.mat'),'model','good_cond')
saveas(figure(1),fullfile(out_dir,'model.jpg'))
